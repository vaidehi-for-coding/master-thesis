...
def get_unexpected_recs(cosine_sim_list, n_liked_items):
    ...
    unexpected_indices_act = [sorted_articles[0][unexpected_indices[0]][0], sorted_articles[0][unexpected_indices[1]][0]]

    return unexpected_indices_act


def get_rec_df(df, artNo_vs_sim):
    for k, v in artNo_vs_sim.items():
        artNo_vs_sim[k] = float(v)

        sorted_d = dict(sorted(artNo_vs_sim.items(), key=operator.itemgetter(1), reverse=True))
        for key in sorted(artNo_vs_sim.items(), key=operator.itemgetter(1), reverse=True)[:10]:
            print(df.loc[key, :])
...
# def get_final_rec_indices(filtered_rec_list, closest_n, unexp_list, query_categories, df):
#     rec_indices = []
#     cos_sim = []
#
#     # aggressively recommend articles from the same category
#     for i in range(len(filtered_rec_list)):
#         count = 0
#         for idx, distance in filtered_rec_list[i][0]:
...
#                 break
#
#     for i in range(len(unexp_list)):
#         rec_indices.append(unexp_list[i])
#         cos_sim.append(0)
#
#     return rec_indices, cos_sim
...
def get_final_rec_indices(filtered_rec_list, closest_n, unexp_list, query_categories, SERENDIPITY, df):
    ...
    # if SERENDIPITY:
    #     random.shuffle(rec_indices)
    #     rec_indices = rec_indices[:8]
    #     rec_indices.extend(unexp_list)
    #     return rec_indices, cos_sim[:10]  # COS_SIM IS NOT SHUFFLED!!!!!!!!!!!!!!!!!!!!!
    # else:
    #     random.shuffle(rec_indices)
    #     return rec_indices[:10], cos_sim[:10]  # COS_SIM IS NOT SHUFFLED!!!!!!!!!!!!!!!!!!!!!
